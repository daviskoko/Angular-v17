...
export class Flutterwave {
  ...
  inlinePay(paymentData: InlinePaymentOptions) {
    const data = {
      ...
      callback: (response: any) => {
        ...
        if (paymentData.callbackContext) {
  ...
  ): Promise<PaymentSuccessResponse | 'closed'> {
    return new Promise((resolve, reject) => {
      paymentData = {
        ...paymentData,
        callback: ($event: object | PaymentSuccessResponse | PromiseLike<PaymentSuccessResponse | "closed">) => {
  ...
  submitToTracker(paymentData: InlinePaymentOptions | AsyncPaymentOptions, response: object, responseTime: number) {
    this.tracker.track({
      paymentData,
      response,
      responseTime,
